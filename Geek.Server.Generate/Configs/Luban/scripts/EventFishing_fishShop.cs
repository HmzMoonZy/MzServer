
//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a tool.
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

using Luban;


namespace HotFix.Cfg
{
public sealed partial class EventFishing_fishShop : Luban.BeanBase
{
    public EventFishing_fishShop(ByteBuf _buf) 
    {
        id = _buf.ReadInt();
        group = _buf.ReadInt();
        purchaseId = _buf.ReadInt();
        nameId = _buf.ReadString();
        buyLimit = _buf.ReadInt();
        {int __n0 = System.Math.Min(_buf.ReadSize(), _buf.Size);goods = new string[__n0];for(var __index0 = 0 ; __index0 < __n0 ; __index0++) { string __e0;__e0 = _buf.ReadString(); goods[__index0] = __e0;}}
    }

    public static EventFishing_fishShop DeserializeEventFishing_fishShop(ByteBuf _buf)
    {
        return new EventFishing_fishShop(_buf);
    }

    /// <summary>
    /// 编号
    /// </summary>
    public readonly int id;
    /// <summary>
    /// 组
    /// </summary>
    public readonly int group;
    /// <summary>
    /// 支付ID<br/>不配则直接领取
    /// </summary>
    public readonly int purchaseId;
    /// <summary>
    /// 目标名称（多语言）
    /// </summary>
    public readonly string nameId;
    /// <summary>
    /// 限购数量
    /// </summary>
    public readonly int buyLimit;
    /// <summary>
    /// 出售道具
    /// </summary>
    public readonly string[] goods;
   
    public const int __ID__ = -2016905699;
    public override int GetTypeId() => __ID__;

    public  void ResolveRef(Tables tables)
    {
    }

    public override string ToString()
    {
        return "{ "
        + "id:" + id + ","
        + "group:" + group + ","
        + "purchaseId:" + purchaseId + ","
        + "nameId:" + nameId + ","
        + "buyLimit:" + buyLimit + ","
        + "goods:" + Luban.StringUtil.CollectionToString(goods) + ","
        + "}";
    }
}

}

